...
其余 Skill 同理，替换脚本路径即可（`resume-agy` / `resume-codex` / `resume-copilot` / `resume-cursor` / `resume-dsh` / `resume-grok` / `resume-kimi` / `resume-opencode` / `resume-qoder` / `resume-zcode`）。
