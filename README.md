...
- 「克隆 xiaosauros/skills-resume 这个仓库，把全部 16 个 Skill 安装到用户级 skills 目录」
...
其余 Skill 同理，替换脚本路径即可（`resume-agy` / `resume-codex` / `resume-copilot` / `resume-cursor` / `resume-dsh` / `resume-grok` / `resume-hermes` / `resume-kilo` / `resume-kimi` / `resume-mimo` / `resume-opencode` / `resume-pi` / `resume-qoder` / `resume-workbuddy` / `resume-zcode`）。

OpenCode、Kilo（kilo.db）、MiMo（mimocode.db）、ZCode 与 Hermes 当前版本均使用 SQLite，Node 实现需要 Node.js 22.5+ 的内置 `node:sqlite`；较低版本 Node 请直接运行对应 Python 脚本。
