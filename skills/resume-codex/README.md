...
接管/恢复 **Codex CLI** 会话：读取本机 `~/.codex` 下的 rollout 会话记录与 session_index，解析消息、工具调用、工具结果与摘要，生成结构化「接管摘要」，供当前模型带着完整上下文继续未完成的工作。
...
└── scripts/
    ├── resume_codex.js   # Node.js 实现（默认）
    └── resume_codex.py   # Python 实现（等价）
```
...
```bash
python -X utf8 scripts/resume_codex.py --list
```
